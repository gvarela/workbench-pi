...
| `/wb-implement [target]` | **Autonomous, observable, steerable** loop over a target's subtree. Workers run in the **background** (promise-based coordinator, non-blocking) so the session stays live — `/agents` works mid-run, the status line shows the current tool, and `/wb-stop` halts it. Every worker gets a deterministic **context pack**: full bead detail (`bd show`), the project **runbook** (detected test command, e.g. `mise exec -- bundle exec rspec`), AGENTS.md (replace-mode subagents can't see it otherwise), and plan-doc pointers. Chain: fresh-context TDD worker → independent `wb-verifier` (bead closes only on PASS, one retry; worker crashes are labeled infra failures, never verified) → next ready leaf, until dry (scope re-checked when dry, HARD_CAP 100). **Scope = whatever you target** — an **epic id** (whole plan), a **PR/phase id** (just that PR, then stops), a **task id**, a **tasks.md path**, or a **substring** (plans discovered **repo-wide**). Scoping uses the target's **descendant closure across dependency AND parent-child edges**, **leaf-preferred** (umbrella parents skipped — their children are the work), intersected with `bd ready`. Epic id read from `tasks.md` (`beads_epic:` frontmatter or `Epic: \`…\`` body). Ambiguous/no match → lists candidates. |
| `/wb-validate [plan]` | Verifier agent validates the implementation against a plan (repo-wide discovery; targeted specs, not the full suite); writes `validation.md`. Non-blocking. |
| `/wb-override` | Escape hatch: toggle the discipline gates off/on for the session |

| `/wb-status` | Show the running loop's progress (done / current task+phase). |
| `/wb-stop [now]` | Halt the loop after the current task; `/wb-stop now` also aborts the in-flight worker (its bead is reopened). |

All agent-running commands are **non-blocking**: they return immediately and the agents run in the background, so the session stays responsive — `/agents` inspects/steers them mid-run and results arrive as notifications. (`/wb-execution` is the exception: it holds the session for its interactive confirm.)
